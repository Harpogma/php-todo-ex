<?php
      ...
      define('DB_USER', getenv('TODOLIST_DB_URL' ?: 'todolist'));
      define('DB_PASS', getenv('TODOLIST_DB_PASS'));
      define('DB_NAME', getenv('TODOLOST_BD_NAME') ?: 'todolist');
...
if (isset($_POST['action'])) {
  switch ($_POST['action']) {
    ...
    case 'toggle':
      ...
      $id = $_POST['id'];
      if (is_numeric($id)) {
        $update = $db->prepare('UPDATE todo SET done = NOT done WHERE id = :id');
        $update->bindParam(':id', $id, PDO::PARAM_INT);
        if (!$update->execute()) {
          die(print_r($update->errorInfo(), true));
        }
      }
    ...
    case 'delete':
      ...
      if (is_numeric($id)) {
        ...
        if (!$delete->execute()) {
          die(print_r($delete->errorInfo(), true));
        }
      }
